...
function drag(elementToDrag, event, options) {
  //log('entered drag');
  if(event.touches && event.touches.length > 1) return false; // only drag for one finger on screen
  options = options || {};
  ...
  if (document.addEventListener) {  // Standard event model
      ...
      document.addEventListener(bTouchMode? "touchend": "mouseup", upHandler, true);
  }
      ...
      elementToDrag.attachEvent(bTouchMode? "touchend": "onmouseup", upHandler);
